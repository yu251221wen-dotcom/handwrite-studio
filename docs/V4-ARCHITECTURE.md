...
全页纸张设置采用“临时投影、确认提交”的两阶段流程。`pageSettingsPreview` 通过 `applyPageSettingsOverride()` 派生有效页面，仅交给预览 Canvas；确认时才把整批页面作为一个 history present 写入，取消则不改变项目。复制边界限定为纸张坐标系和背景展示，不触碰文字、行偏移、涂改、签名或选择状态。

高 DPI 导出改为单页生产/消费。前端复用一个导出 Canvas，逐页编码并释放；PDF 使用 Session 隔离的 job API 接收单页 PNG，后端逐页写入 A4 PDF。取消会同时终止前端循环和后端 job，避免累积完整文档的高分辨率 Canvas 或 Blob。

OCR Lite 作为输入适配器加入，输出仍为 `DocumentBlock[]`，不会建立第二套排版或绘制路径。详细隐私、校对、PDF 清理、表格线和罗马数字策略见 [V4.3 OCR Lite 与稳定工作流](V4.3-OCR-LITE.md)。
