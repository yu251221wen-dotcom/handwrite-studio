...
墨迹排版台是本地优先、可选在线临时会话的通用 DOCX 手写排版器。V4.2.1 在 V4.2 基础上优化长文档预览性能与纸线分页槽位利用；原有多页 Canvas、逐行编辑、Undo/Redo 与高 DPI 导出均保留。
...
OCR、PaddleOCR、扫描 PDF OCR、AI/LLM 字段匹配、Hough Transform、自动透视校正、四角透视拖拽、弯曲纸张、SVG Path 笔画扰动、自动圈画与 AI 生成笔迹仍未实现。V4.2 横线检测只支持无透视、基本水平的横线纸；自动涂改仅为低概率、安全文本规则，不是语义纠错或 AI 生成。
