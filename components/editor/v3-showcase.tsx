...
import { streamPages } from "@/lib/export/stream-export";
...
function project(noTemplateMode: ProjectState["noTemplateMode"]): ProjectState {
  const base: ProjectState = { schemaVersion: 3, projectVersion: "4.3.0", id: `showcase-${noTemplateMode}`,
    ...
    fieldMappings: [], pages: [], handwriting, inkStyle: DEFAULT_INK_STYLE, correctionStyle: DEFAULT_CORRECTION_STYLE, footerMode: "generated", tableLineMode: "auto",
...
export function V3Showcase() {
  ...
  const exportProject = async (value: ProjectState, stem: string) => {
    setStatus(`正在生成 ${stem}…`); await document.fonts.ready;
    const { jobId } = await apiJson<{ jobId: string }>("/api/export/pdf-jobs", { method: "POST" }); const canvas = document.createElement("canvas");
    try {
      await streamPages({ total: value.pages.length,
        render: async (index) => { const page = value.pages[index]; renderCompositeCanvas(canvas, { page, pageCount: value.pages.length, font, background, handwriting,
          inkStyle: value.inkStyle, correctionStyle: value.correctionStyle, footerMode: value.footerMode, tableLineMode: value.tableLineMode,
          seed: value.seed, documentId: value.document.id, patientFields: {}, fieldTextById: Object.fromEntries(V3_SHOWCASE_BLOCKS.map((block) => [block.blockId, block.sourceText])) }, A4_PIXELS[300].width, A4_PIXELS[300].height);
          const blob = await canvasBlob(canvas); canvas.width = 1; canvas.height = 1; return blob; },
        consume: async (blob, index) => { const body = new FormData(); body.append("file", blob, `page-${index + 1}.png`);
          const response = await apiFetch(`/api/export/pdf-jobs/${jobId}/pages/${index}`, { method: "PUT", body }); if (!response.ok) throw new Error("PDF 页面暂存失败");
          if (index === 0) download(blob, `${stem}-300dpi.png`); },
        onProgress: (progress) => setStatus(`正在生成 ${progress.current} / ${progress.total} 页`),
      });
      const body = new FormData(); body.append("name", stem); const response = await apiFetch(`/api/export/pdf-jobs/${jobId}/complete`, { method: "POST", body });
      if (!response.ok) throw new Error("PDF 生成失败"); download(await response.blob(), `${stem}.pdf`); setStatus(`${stem} 已生成`);
    } catch (error) { void apiFetch(`/api/export/pdf-jobs/${jobId}`, { method: "DELETE" }); setStatus(error instanceof Error ? error.message : "导出失败"); }
    finally { canvas.width = 1; canvas.height = 1; }
  };
  return <main className="min-h-screen bg-[#f3f6f8] px-5 py-8 text-slate-900 sm:px-8"><div className="mx-auto max-w-[1500px]">
    <header className="mb-7 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold tracking-[.18em] text-[#287e86]">V4.3 PUBLIC SHOWCASE</p><h1 className="mt-1 text-2xl font-semibold">通用文档排版与 Block 结构验收</h1><p className="mt-2 max-w-3xl text-sm text-slate-500">全部内容为脱敏合成数据。展示保留原文结构、简化正文、结构块和多页连续预览。</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" asChild><Link href="/">返回编辑器</Link></Button><Button onClick={() => void exportProject(preserve, "v4-preserve-structure")}><Download />保留结构 PDF</Button><Button onClick={() => void exportProject(simplified, "v4-simplified")}><Download />简化正文 PDF</Button></div></header>
